...
//! # wordlist
//! Contains pricklybird wordlist and reverse wordlist hashmap.
...
/// Letters are expected to be ASCII codepoints.
pub const fn word_hash(word_first_letter: u8, word_last_letter: u8) -> usize {
    (word_first_letter as usize - 97) + (word_last_letter as usize - 97) * 26
}
...
const fn generate_hash_table(byte_wordlist: &[[u8; 4]; 256]) -> [u8; HASH_TABLE_SIZE] {
    let mut result = [0u8; HASH_TABLE_SIZE];
